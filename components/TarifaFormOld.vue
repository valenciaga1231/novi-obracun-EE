    ...
    <div class="tarifa-component">
        <div class="tarifa-content">
            <div class="p-float-label">
                <label for="input-tariff">Izberi tarifo za stari račun(brez DDV)</label>
            ...
            <div v-if="settings.tip_starega_obracuna !== null">
                <div v-if="settings.tip_starega_obracuna === 'VT+MT'" style="display: flex; flex-direction: column">
                    <span class="flex-auto" style="margin-bottom: 10px">
                        ...
                        <InputNumber inputId="number-input-vt" v-model="settings.vrednosti_tarif_old.VT" :minFractionDigits="6" locale="de-DE" suffix="                      EUR/kWh" fluid />
                    </span>
                    <span class="flex-auto">
                        <label for="number-input-mt" class="font-bold mb-2">MT</label>
                        <InputNumber inputId="number-input-mt" v-model="settings.vrednosti_tarif_old.MT" :minFractionDigits="6" locale="de-DE" suffix="                      EUR/kWh" fluid />
                    </span>
                </div>
                <div v-if="settings.tip_starega_obracuna === 'ET'">
                    <span class="p-float-label">
                        <label for="number-input-et">ET</label>
                        <InputNumber inputId="number-input-et" v-model="settings.vrednosti_tarif_old.ET" :minFractionDigits="6" locale="de-DE" suffix="                      EUR/kWh" fluid />
                    </span>
                </div>
            </div>
        </div>
...
export default {
    setup() {
        ...
        watch(
            () => settings.value.vrednosti_tarif_old,
            (val) => {
                // if (useIsTable().value) sumAllCosts();
                localStorage.setItem("vrednosti_tarif_old", JSON.stringify(val));
            },
            { deep: true }
        );
